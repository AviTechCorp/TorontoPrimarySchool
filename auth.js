...
const admissionsTeamFields = document.getElementById('admissions-team-fields'); // NEW FIELD REF
...
admissionsTeamFields.style.display = 'none'; // HIDE NEW FIELD
...
switch (selectedRole) {
 ...
 break;
 case 'admissions-team': // SHOW NEW FIELD
 admissionsTeamFields.style.display = 'block';
 break;
 case 'admin':
...
} else if (role === 'admissions-team') { // NEW DATA COLLECTION
...
loginSubmitButton.addEventListener('click', async function(event) {
 ...
 if (userDocSnap.exists()) {
  ...
  switch(role) {
   ...
   break;
   case 'admissions-team': // NEW REDIRECTION
   window.location.href = "admission-team-portal.html";
   break;
   case 'admin':
